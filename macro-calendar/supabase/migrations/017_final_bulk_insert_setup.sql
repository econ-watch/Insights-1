-- Add unique constraint and bulk insert function

-- 1. Add unique constraint to releases table (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'releases_indicator_release_unique'
    ) THEN
...
-- 2. Create function for bulk inserting releases with conflict handling
